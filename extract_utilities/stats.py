...
class Stats:
    def __init__(self, stats_types=None, data_since=None, collect=None, expand=None, season_level_request=None) -> None:
    ...
    def all_player_stats(self, player_type, level="season_by_season"):
        ...
        if self.collect:
            for stat_type in self.stats_types:
                ...
                settings.temp_files_to_df(wildcard="*.csv", fp_temp=temp_directory, fp_final=directory, fn=player_type + "s")
        return True

    def player_stats_all_seasons(self, player_id):
        player = self.player_seasons[self.player_seasons["person_id"].isin([player_id])]
        seasons = ndr.generate_season_array(str(player.season.amin.values[0]), str(player.season.amax.values[0]))
        for season in seasons:
            self._player_stats_by_season(player_id=player_id, season=season)
                       
    def _player_stats_by_season(self, player_id, season):
        stats_types = ",".join(self.stats_types) if isinstance(self.stats_types, list) else self.stats_types
        url = f"https://statsapi.web.nhl.com/api/v1/people/{player_id}/stats/?stats={stats_types}&season={season}"
        body = ndr.get_request(url)
        if not body:
            return None
        stats = {b["type"]["displayName"]: self._sub_df_transform(player_id=player_id, splits=b["splits"])
                 for b in body["stats"] if b and b.get("type", False)}
        
        for k, v in stats.items():
            settings.write_df_to_storage(df=v, fp=f"data/stats/{k}_temp", fn=f"{player_id}_{season}", file_type="csv")
        return True
    ...
    def player_stats(self, player_id):
        ...
        for k, v in stats.items():
            settings.write_df_to_storage(df=v, fp=f"data/stats/{k}_temp", fn=f"{player_id}", file_type="csv")
        return True
